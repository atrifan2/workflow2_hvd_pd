...
md_path = os.path.join(base_path, 'MD_exps/PLProA') 
...
pdb_file = os.path.join(md_path, 'pdb/plproA_last.pdb') 
top_file = os.path.join(md_path, 'pdb/prot.prmtop')
ref_pdb_file = None #os.path.join(md_path, 'pdb/prot.pdb')
...
#                print "Running from checkpoint %s for %d ns" % (outlier_list[i], LEN_iter) 
    ...
    def generate_aggregating_stage(): 
        ...
        t2.executable = ['/sw/summit/ibm-wml-ce/anaconda-base/envs/ibm-wml-ce-1.7.0-2/bin/python']
    ...
    def generate_ML_stage(num_ML=1): 
        ...
        for i in range(num_ML): 
            ...
            t3.pre_exec += ['module unload prrte', 'module unload python', 'module load xl', 'module load xalt', 'module load spectrum-mpi', 'module load cuda','module list']
            ...
            t3.pre_exec += ['mkdir -p {0}/benchmarks && cd {0}'.format(cvae_dir)]
            ...
            t3.pre_exec += ['. /sw/summit/ibm-wml-ce/anaconda-base/etc/profile.d/conda.sh',
                    'source /sw/summit/ibm-wml-ce/anaconda-base/etc/profile.d/conda.sh',
                    'conda deactivate', 'conda deactivate', 'conda activate /sw/summit/ibm-wml-ce/anaconda-base/envs/ibm-wml-ce-1.7.0-2']
            t3.pre_exec += ['export CUDA_VISIBLE_DEVICES="0,1,2,3,4,5"']
            t3.pre_exec += ['jsrun --erf_input /gpfs/alpine/med110/scratch/atrifan2/covid19/PLPro/entk_cvae_md_hvd/RANKFILE /sw/summit/ibm-wml-ce/anaconda-base/envs/ibm-wml-ce-1.7.0-2/bin/python %s/cvae/train_cvae.py -f ../bytes-train.tfrecords --dim %s' % (cvae_path, dim)]
            #t3.pre_exec += ['/sw/summit/ibm-wml-ce/anaconda-base/envs/ibm-wml-ce-1.7.0-2/bin/ddlrun /sw/summit/ibm-wml-ce/anaconda-base/envs/ibm-wml-ce-1.7.0-2/bin/python %s/cvae/train_cvae.py -f ../bytes-train.tfrecords --dim %s' % (cvae_path, dim)]
            t3.executable = ['date'] #t3.executable = ['/sw/summit/ibm-wml-ce/anaconda-base/envs/ibm-wml-ce-1.7.0-2/bin/python']
    ...
    def generate_interfacing_stage(): 
        ...
        t4.pre_exec += ['module unload prrte', 'module unload python', 'module load xl', 'module load xalt', 'module load spectrum-mpi', 'module load cuda','module list']
        t4.pre_exec += ['. /sw/summit/ibm-wml-ce/anaconda-base/etc/profile.d/conda.sh', 
                    'source /sw/summit/ibm-wml-ce/anaconda-base/etc/profile.d/conda.sh',
                    'conda deactivate', 
                    'conda deactivate', 
        ...
        t4.pre_exec += ['export PYTHONPATH=%s/CVAE_exps:%s/CVAE_exps/cvae:$PYTHONPATH' % (base_path, base_path)]
