...
if DEBUG:
    print ("==={}:{}===".format(frameinfo.lineno, time.time()), flush=True)
...
print ("Using model {} with loss {}".format(model_best, loss_model_best), flush=True)
...
print ('Model latent dimension: %d' % model_dim, flush=True)
...
while True: 
    ...
    print('dimension = {0}, eps = {1:.2f}, number of outlier found: {2}'.format(
        model_dim, eps, n_outlier), flush=True)
...
if DEBUG: 
    print (outlier_list_uni, flush=True)
...
print ('Writing outliers in %s' % outliers_pdb_path, flush=True)
...
for outlier in outlier_list_uni: 
    ...
    if not os.path.exists(outlier_pdb_file): 
        print ('Found a new outlier# {} at frame {} of {}'.format(outlier, num_frame, traj_file), flush=True)

        outlier_pdb = write_pdb_frame(traj_file, pdb_file, num_frame, outlier_pdb_file)
        print ('     Written as {}'.format(outlier_pdb_file), flush=True)
...
for outlier in outliers_list: 
    if outlier not in new_outliers_list: 
        print ('Old outlier {} is now connected to a cluster and removing it from the outlier list '.format(os.path.basename(outlier)), flush=True)
...
for checkpnt in checkpnt_list: 
    ...
    if not os.path.exists(checkpnt_filepath): 
        ...
        print ([os.path.basename(os.path.dirname(checkpnt)) in outlier for outlier in outliers_list], flush=True)
...
if DEBUG: 
    print (restart_checkpnts, flush=True)



if DEBUG: 
    print (restart_pdbs, flush=True)
...
print (restart_points, flush=True)
